...
```{r, load_pck1, warning=TRUE, message=TRUE}
library(dplyr)
library(ggplot2)
```
...
df_01%>% 
  ggplot(mapping = aes (x = obs_id))+ 
  geom_line(mapping = aes (y = x))+
  geom_point(mapping = aes (y = x))

```
Yes, there are some values which are greater than 10 or less than -10.
...
df_01%>% 
  ggplot()+ 
  geom_histogram(mapping = aes (x = x), bins = 25)

```

The measurement values are mostly concentrated between -10 to 10. The shape of the distribution looks like a normal distribution.
### 1c)
...
In normal distribution, the mean, median and mode are equal. However, in the summary, the mean and median of x are not equal. So, probably our distribution is not exactly the normal distribution. 
### 1d)
...
mutate (rolling_avg = cummean (x))%>% 
  ggplot()+
  geom_line(mapping = aes (x = obs_id, y=rolling_avg))


```
The initial observations, especially the ones less than 50 are the most variables. However, the rolling average seems to have less variance and steady-out as the number of observations increase.
...
```
The sample average after 5 observations is 0.2509444.
...
```
The sample average after 45 observations is 1.242825.
## Problem 2
...
The sigma likelihood has a limitation of having a lower bound at 0. We use log transformation of sigma which is varphi to overcome this limitation and have values from negative to positive infinity. 
...
$$
p(\mu,\varphi|x) \propto \prod_{n=1}^{N} \left( \mathrm{normal}\left( x_n \mid \mu,g^{-1} \left( \varphi \right) \right) \right) \times p\left(\mu\right) \times p\left(g^{-1}( \varphi \right))
$$
...
### bounds on varphi
varphi_lwr <- qexp(0.01, rate = info_inform_N05$sigma_rate)

varphi_upr <- qexp(0.99, rate = info_inform_N05$sigma_rate)
...
$\varphi$ values over 4 seems to be completly ruled out after 5 observations. Values between -1 to 0 of $\mu$ appears to be the most probable ones. The prior mean was -0.5, which seem to be quite close to posterior mean. 
...
```
The log posterior becomes more concentrated around the $\mu$ and $\varphi$ value that has the highest probability (MAP) as the sample size increases. 
Yes it does change to some extent as the number of observation increase that is because the likelihood data become more impactful on posterior comparing to prior data as the number of samples goes up. 
Yes, the uncertainty in parameters decreases with higher number of sample (the contour became smaller)
...
#### SOLUTION
```{r, solution_03_b_a, eval=TRUE}
map_res_01

```
```{r, solution_03_b_b, eval=TRUE}
map_res_02

```
Both identified optimal parameters and hessian matrix are the same for both initial guesses. The only difference is function gradients. 
...
```
As the number of observation inreases, the posterior mean of $\mu$ increases as well. Which mean the posterior $\mu$ is mostly affected by likelihood. However the sigma mean almost remain the same with minor fluctuations, but based on the below histogram the sigma mode is going over 5 as the number of sample increases. 




```{r, solution_03f_d, eval=TRUE}
post_samples_inform_N05 %>% 
  ggplot(mapping = aes(x = mu)) +
  geom_histogram(bins = 30)
```

```{r, solution_03f_e, eval=TRUE}
post_samples_inform_N05 %>% 
  ggplot(mapping = aes(x = sigma)) +
  geom_histogram(bins = 30)
```
```{r, solution_03f_f, eval=TRUE}
post_samples_inform_N45 %>% 
  ggplot(mapping = aes(x = mu)) +
  geom_histogram(bins = 30)
```
```{r, solution_03f_g, eval=TRUE}
post_samples_inform_N45 %>% 
  ggplot(mapping = aes(x = sigma)) +
  geom_histogram(bins = 30)
```
```{r, solution_03f_h, eval=TRUE}
post_samples_inform_N405 %>% 
  ggplot(mapping = aes(x = mu)) +
  geom_histogram(bins = 30)
```
```{r, solution_03f_i, eval=TRUE}
post_samples_inform_N405 %>% 
  ggplot(mapping = aes(x = sigma)) +
  geom_histogram(bins = 30)
```
...
n_total = nrow(post_samples_inform_N05)  
positive_mu_N05 = post_samples_inform_N05 %>% filter (mu > 0)
n_positive_mu_N05 = nrow (positive_mu_N05)
p_positive_N05 =  n_positive_mu_N05 / n_total 
p_positive_N05
```
```{r, solution_03g_b, eval=TRUE}
n_total = nrow(post_samples_inform_N45)  
positive_mu_N45 = post_samples_inform_N45 %>% filter (mu > 0)
n_positive_mu_N45 = nrow (positive_mu_N45)
p_positive_N45 =  n_positive_mu_N45 / n_total 
p_positive_N45
```
```{r, solution_03g_c, eval=TRUE}
n_total = nrow(post_samples_inform_N405)  
positive_mu_N405 = post_samples_inform_N405 %>% filter (mu > 0)
n_positive_mu_N405 = nrow (positive_mu_N405)
p_positive_N405 =  n_positive_mu_N405 / n_total 
p_positive_N405
```
The probability of having positive $\mu$ for N = 5 is 34.3%, for N=45 is 85.7% and for n=405 is 99.7%. 


```{r, solution_03g_d, eval=TRUE}
qnorm(0.05, mean = mean(post_samples_inform_N05$sigma), sd = sd(post_samples_inform_N05$sigma))
qnorm(0.95, mean = mean(post_samples_inform_N05$sigma), sd = sd(post_samples_inform_N05$sigma))
```
```{r, solution_03g_e, eval=TRUE}
qnorm(0.05, mean = mean(post_samples_inform_N45$sigma), sd = sd(post_samples_inform_N45$sigma))
qnorm(0.95, mean = mean(post_samples_inform_N45$sigma), sd = sd(post_samples_inform_N45$sigma))
```
```{r, solution_03g_f, eval=TRUE}
qnorm(0.05, mean = mean(post_samples_inform_N405$sigma), sd = sd(post_samples_inform_N405$sigma))
qnorm(0.95, mean = mean(post_samples_inform_N405$sigma), sd = sd(post_samples_inform_N405$sigma))
```
## Problem 4
...
```
```{r, solution_04a_2, eval=TRUE}
df_02_high %>%
  ggplot(mapping = aes(x=x))+
  geom_point(mapping = aes (y=y))
```
I guess the low noise data might correspond to parabolic function. 
### 4b)
...
mod_low_quad <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4), data = df_02_low)
...
mod_high_linear <- lm (y ~ x, data = df_02_high)

mod_high_quad <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4), data = df_02_high)

mod_high_8th <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), data = df_02_high)
```
...
```
for low noise data, the quadratic model has the highest R squared while it is very close to 8th as well. in high noise data, the 8th model has the highest r squared. 
### 4d)
...
mod_low_quad_fold_01 <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4), data = low_noise_train_fold_01)

mod_low_quad_fold_02 <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4), data = low_noise_train_fold_02)

mod_low_quad_fold_03 <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4), data = low_noise_train_fold_03)

mod_low_quad_fold_04 <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4), data = low_noise_train_fold_04)

mod_low_quad_fold_05 <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4), data = low_noise_train_fold_05)
```

The 8th order polynomial fits should be specified below.  

```{r, solution_04f_b, eval=TRUE}
mod_low_8th_fold_01 <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), data = low_noise_train_fold_01)

mod_low_8th_fold_02 <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), data = low_noise_train_fold_02)

mod_low_8th_fold_03 <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), data = low_noise_train_fold_03)

mod_low_8th_fold_04 <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), data = low_noise_train_fold_04)

mod_low_8th_fold_05 <- lm (y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), data = low_noise_train_fold_05)
```
...
cv_low_quad_fold_train_rmse[1] <- modelr::rmse(mod_low_quad_fold_01, low_noise_train_fold_01)
cv_low_quad_fold_train_rmse[2] <- modelr::rmse(mod_low_quad_fold_02, low_noise_train_fold_02)
cv_low_quad_fold_train_rmse[3] <- modelr::rmse(mod_low_quad_fold_03, low_noise_train_fold_03)
cv_low_quad_fold_train_rmse[4] <- modelr::rmse(mod_low_quad_fold_04, low_noise_train_fold_04)
cv_low_quad_fold_train_rmse[5] <- modelr::rmse(mod_low_quad_fold_05, low_noise_train_fold_05)

cv_low_quad_fold_train_rmse
```

```{r, solution_04g_b, eval=TRUE}
cv_low_8th_fold_train_rmse <- c(modelr::rmse (mod_low_8th_fold_01, low_noise_train_fold_01))
cv_low_8th_fold_train_rmse [1] <- modelr::rmse (mod_low_8th_fold_01, low_noise_train_fold_01)
cv_low_8th_fold_train_rmse[2] <- modelr::rmse(mod_low_8th_fold_02, low_noise_train_fold_02)
cv_low_8th_fold_train_rmse[3] <- modelr::rmse(mod_low_8th_fold_03, low_noise_train_fold_03)
cv_low_8th_fold_train_rmse[4] <- modelr::rmse(mod_low_8th_fold_04, low_noise_train_fold_04)
cv_low_8th_fold_train_rmse[5] <- modelr::rmse(mod_low_8th_fold_05, low_noise_train_fold_05)

cv_low_8th_fold_train_rmse

```
```{r, solution_04g_c, eval=TRUE}
sum_quad = cv_low_quad_fold_train_rmse[1] + cv_low_quad_fold_train_rmse[2] + cv_low_quad_fold_train_rmse[3] + cv_low_quad_fold_train_rmse[4] + cv_low_quad_fold_train_rmse[5]
avg_quad = sum_quad /5
avg_quad
```

```{r, solution_04g_d, eval=TRUE}
sum_8th = cv_low_8th_fold_train_rmse[1] + cv_low_8th_fold_train_rmse[2] + cv_low_8th_fold_train_rmse[3] + cv_low_8th_fold_train_rmse[4] + cv_low_8th_fold_train_rmse[5]
avg_8th = sum_8th /5
avg_8th
```
The 8th fold has the lower average rmse of 0.1749371, while the quad fold has the higher average rmse of 0.1969927. Therefore, the 8th fold has the lower average rmse. 
### 4h)
...
cv_low_quad_fold_test_rmse[1] <- modelr::rmse(mod_low_quad_fold_01, low_noise_test_fold_01)
cv_low_quad_fold_test_rmse[2] <- modelr::rmse(mod_low_quad_fold_02, low_noise_test_fold_02)
cv_low_quad_fold_test_rmse[3] <- modelr::rmse(mod_low_quad_fold_03, low_noise_test_fold_03)
cv_low_quad_fold_test_rmse[4] <- modelr::rmse(mod_low_quad_fold_04, low_noise_test_fold_04)
cv_low_quad_fold_test_rmse[5] <- modelr::rmse(mod_low_quad_fold_05, low_noise_test_fold_05)

cv_low_quad_fold_test_rmse
```
```{r, solution_04h_b, eval=TRUE}
cv_low_8th_fold_test_rmse <- (modelr::rmse(mod_low_8th_fold_01, low_noise_test_fold_01))
cv_low_8th_fold_test_rmse[1] <- modelr::rmse(mod_low_8th_fold_01, low_noise_test_fold_01)
cv_low_8th_fold_test_rmse[2] <- modelr::rmse(mod_low_8th_fold_02, low_noise_test_fold_02)
cv_low_8th_fold_test_rmse[3] <- modelr::rmse(mod_low_8th_fold_03, low_noise_test_fold_03)
cv_low_8th_fold_test_rmse[4] <- modelr::rmse(mod_low_8th_fold_04, low_noise_test_fold_04)
cv_low_8th_fold_test_rmse[5] <- modelr::rmse(mod_low_8th_fold_05, low_noise_test_fold_05)

cv_low_8th_fold_test_rmse
```
```{r, solution_04h_c, eval=TRUE}
sum_quad_test = cv_low_quad_fold_test_rmse[1] + cv_low_quad_fold_test_rmse[2] + cv_low_quad_fold_test_rmse[3] + cv_low_quad_fold_test_rmse[4] + cv_low_quad_fold_test_rmse[5]
avg_quad_test = sum_quad_test /5
avg_quad_test
```
```{r, solution_04h_d, eval=TRUE}
sum_8th_test = cv_low_8th_fold_test_rmse[1] + cv_low_8th_fold_test_rmse[2] + cv_low_8th_fold_test_rmse[3] + cv_low_8th_fold_test_rmse[4] + cv_low_8th_fold_test_rmse[5]
avg_8th_test = sum_8th_test /5
avg_8th_test
```

it look likes the quad has the lowest average cross-validation rmse (0.3459554).
