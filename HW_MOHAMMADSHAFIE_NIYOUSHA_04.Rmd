...
It seems that there is negative linear relationship between the input and the response.
...
lm_logpost <- function(unknowns, my_info)
{
  ...
  log_prior <- log_prior_sigma + log_prior_beta
...
No, posterior result does not depend on the first guess. 
...
1 - pnorm(0, mean = mean(post_samples_01$beta_01), sd = sd(post_samples_01$beta_01))
```
Based on the above calculation there is no slope greater than 0. 
...
```
Yes the number of columns is equal to the number of predition points. 
### 2c)
...
error_01_post_01 <- c(post_pred_samples_01$Umat [, 1] - train_01$y)

### error of the second posterior sample
error_01_post_02 <-  c(post_pred_samples_01$Umat [, 2] - train_01$y)

### error of the third posterior sample
error_01_post_03 <-  c(post_pred_samples_01$Umat [, 3] - train_01$y)

```

The response predict make the prediction intervals which usually much wider than confidence interval (uncertainty interval from the mean trend).
...
The MAE values are pretty close but they are not exactly the same. That is because the predicted posteriors are not exaclty the same. 
...
The reason that MAE is unertain is due to uncertainty in the mean trend. Since we calculate the MAE based on the data we got from the difference between the observations and the mean trend, the MAE is uncertain as well. 
...
ggplot()+
  geom_point (data = train_02, mapping = aes(x=x, y=y), color = 'navy')+
  geom_point (data = test_02, mapping = aes(x=x, y=y), color = 'red')
...
There are 150 rows in design matrix and 50 rows in test matrix. 
...
### 
```
Based on the visilization the models with J>5-6 seems to overfit the data since while the training data shows lower amount of error, the corresponding test data shows higher amount of error. Therefore the model seems to overfit the data with the training set. 
...
```
Based on current visulization, J = 5 seems to be the best model. That is because it has the highest weight comapring to other models. 
...
No, my conclusion were not exactly the same, The MAE graph for training set and testing test seems that models with j<2 are not a good fit, therefore, I was thinking J = 10 is the best options, however the weights graph showed that my conclusion was not right and the best model in J=2. 
...
$$
{\mu}_{n} = {\beta}_{0} + {\beta}_{1} {x}_{n}
$$
$$
{\eta}_{n} = logit({\mu}_{n}) = log(\dfrac {{\mu}_{n}}{1-{\mu}_{n}})
$$
$$
{\mu}_{n} = logit^{-1}({\eta}_{n}) = \dfrac {exp({\eta}_{n})}{1-exp({\eta}_{n})}
$$
$$
{y}_{n}\mid{\mu}_{n} = {y}_{n}\mid{\beta}_{0},{\beta}_{1},{x}_{n} = \prod_{0}^{D} p ({y}_{n}\mid {X}_{n:}\beta)
$$
$$
p(\beta)=\prod_{0}^{D} (normal({\beta}_{n} \mid {\mu}_{\beta}, {\tau}_{\beta}))
$$
$$
Complete\space probability \space for \space logistic \space regression = \\\prod_{0}^{D} (Bernouli (y_{n}\mid logit ^{-1} ({X}_{n:}\beta))) (normal({\beta}_{n} \mid {\mu}_{\beta}, {\tau}_{\beta}))
$$
...
No, as dicussed before, initial guess does not have any impacr on the results
...
Beta (mode)
```{r, solution_04d_b_a, eval=TRUE}
quantile(laplace_03_line$mode, probs = 0.0275)
quantile(laplace_03_line$mode, probs = 0.9725)
```
```{r, solution_04d_b_b, eval=TRUE}
quantile(laplace_03_cube$mode, probs = 0.0275)
quantile(laplace_03_cube$mode, probs = 0.9725)
```
var_matrix
```{r, solution_04d_b_c, eval=TRUE}
quantile(laplace_03_line$var_matrix, probs = 0.0275)
quantile(laplace_03_line$var_matrix, probs = 0.9725)
```
```{r, solution_04d_b_d, eval=TRUE}
quantile(laplace_03_cube$var_matrix, probs = 0.0275)
quantile(laplace_03_cube$var_matrix, probs = 0.9725)
```
Mode(beta) have zero in its 95% uncetrainty interval in both line and cubic model. Also, var_matrix has zero in cubic model.
...
laplace_03_line$log_evidence / laplace_03_cube$log_evidence
```

since the bayes factor is 0.97, it means that the data favors the cubic model.
...
### 
```
parameter (1 and 3) & (2 and 4) seem to be correlated with correlation factor of  -0.84995947 and -0.6956232 respectively. Also parameters (1 and 2) with correlation factor of -0.001536955 are non-correlated. 
## Problem 05
...
post_glm_pred_samples <- function(Xnew, Bmat)
{
  ...
  # calculate the event probability
  mu_mat <-boot::inv.logit(as.matrix(eta_mat))
...
###
```
there are 100 rows and 4 columns in each matrix. 
...
Linear relationship seem to be more inline with the observation.
...
post_pred_summary_03_line_b <- post_pred_summary_03_line %>% mutate (pred_y = ifelse(post_pred_summary_03_line$mu_avg > 0.5,1,0))

post_pred_summary_03_cube_b <- post_pred_summary_03_cube %>% mutate (pred_y = ifelse(post_pred_summary_03_cube$mu_avg > 0.5,1,0))
```
...
xtabs(~ y + pred_y, post_pred_summary_03_line_c)
...
FP=13
FP=40
Note: I used the function of count initially and I got result, but when I was knitting, I got and error which made knitting halted. 
However this was my code with count: count(post_pred_summary_03_line_c,  c("y","pred_y"))

```{r, solution_05f_b, eval=TRUE}
### confusion matrix for the cubic relationshiop
xtabs(~ y + pred_y, post_pred_summary_03_cube_c)
```
TP=34
TN=16
FP=12
...
My code with count was this: count(post_pred_summary_03_cube_c, c ("y", "pred_y" ))