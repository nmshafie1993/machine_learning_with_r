...
```{r, load_packages, warning=TRUE, message=TRUE}
library(dplyr)
library(ggplot2)
```
...
$$ 
Log(p(H|AB,\mu)) = Log (\dfrac{AB!}{H!\times (AB-H)!}) + H\times log(\mu) + (AB-H)\times log(1 - \mu) \\
So: \\
Log(p(H|AB,\mu)) ∝ H\times log(\mu) + (AB-H)\times log(1 - \mu)
$$
...
tibble::tibble(mu = seq(0.1, 0.6, by = 0.005 )) %>% 
mutate(log_lik =H*log(mu) + (AB-H)*log(1 - mu)) %>% 
ggplot() +
geom_line(mapping = aes(x = mu, y = log_lik))+
geom_vline(xintercept = player_mle, color = "red")
```
The MLE corresponds to the maximimum point in log-likelihood plot. 
...
tibble::tibble(mu = seq(0.1, 0.6, by = 0.005 )) %>% 
  mutate(log_lik = log_binom_pmf_unnorm(H, AB, mu )) %>% 
  ggplot() +
geom_line(mapping = aes(x = mu, y = log_lik))+
geom_vline(xintercept = player_mle, color = "red")
```
...
{log_lik =events*log(prob) + (trials-events)*log(1 - prob)+lchoose(trials,events)
return(log_lik)
}
```
...
tibble::tibble(mu = seq(0.1, 0.6, by = 0.005 )) %>%
  mutate(log_lik = log_binom_pmf(H, AB, mu )) %>%
   ggplot() +
geom_line(mapping = aes(x = mu, y = log_lik))+
geom_vline(xintercept = player_mle, color = "red")
...
Yes it is at the same location as the un-normalized log-likelihood
...
tibble::tibble(mu = seq(0.1, 0.6, by = 0.005)) %>% 
  mutate(log_lik = dbinom(H, AB, mu, log = TRUE )) %>%
  ggplot() +
geom_line(mapping = aes(x = mu, y = log_lik))+
geom_vline(xintercept = player_mle, color = "red")
```
...
coin_probs <- dbinom(0:5, 11, 0.25, log = FALSE)
# print to screen
coin_probs
# length?
length(coin_probs)
...
The first element correspond to probability of observing zero heads out of 11 trials. 
...
```{r, solution_02f}
pbinom(5, 11, 0.25, log=FALSE)
```
The results from pbinom() and manual approach using sum() and dbinom are the same. 
...
```{r, solution_02g_a}
#175 to 195 hits
pbinom(195, 602, 0.3, log=FALSE) - pbinom(174, 602, 0.3, log=FALSE)
```


```{r, solution_02g_b}
#165 to 205 hits
pbinom(205, 602, 0.3, log=FALSE) - pbinom(164, 602, 0.3, log=FALSE)
...
$$ 
log (Beta(\mu|a,b)) \propto log(\mu)^{a-1}+log(1-\mu)^{b-1} = (a-1)log(\mu)+(b-1)log(1-\mu)
$$
...
Log(Likelihood)= Log(p(H|AB,\mu)) ∝ H\times log(\mu) + (AB-H)\times log(1 - \mu)\\
Log(Prior) = log (Beta(\mu|a,b)) \propto (a-1)log(\mu)+(b-1)log(1-\mu)\\
Log(Likelihood) + Log(Prior) = (H+a-1)\times log(\mu)+(AB-H+b-1)\times log(1 - \mu)\\
H+a = a_{new}\\
AB-H+b = b_{new}\\
Log(Likelihood) + Log(Prior)= (a_{new}-1)\times log(\mu)+(b_{new}-1)\times log(1 - \mu)\\
Therefore:\\
Log (Posterior) = log (Beta(\mu|a_{new},b_{new}) ∝ log(\mu^{a_{new}-1})+log(1 - \mu)^{(b_{new}-1)}
$$
...
```{r, solution_03d_a}
# 0.05 quantiles
qbeta(0.05, shape1 = a_prior, shape2 = b_prior)
```


```{r, solution_03d_b}
#0.95 quantiles
qbeta(0.95, shape1 = a_prior, shape2 = b_prior)

```
The MLE based on the first 30 at bats is 0.2 which is less than 0.05 quantile. This shows the initial performance of the player were lower than 0.05 quantile. 
The MLE based on all 602 bats in the season is 0.31 between 0.05 and 0.95 quantile and it is included in 90% credible interval. The MLE  value is closer to 0.95 credible value. 
...
a_post_player <- H + a_prior
b_post_player <- AB - H + b_prior
```
...
post_mean_start 
```
The posterior mean in the 30 first observations is closer to prior mean (~ 0.278) and far from the corresponding MLE (0.2). The reason for that is the number of observation is still low and the posterior mean is being governed more by prior mean rather than obsevation (corresponding MLE). Therefore the prior mean still has a higher impact on post mean and their value are closer.

```{r, solution_03f_b, eval=TRUE}
post_mean_player <- a_post_player / (a_post_player+b_post_player)
post_mean_player
```
The posterior mean in the 602 at bats is 0.306 which is closer to corresponding MLE. This shows that as the number of observations get higher, corresponding MLE has higher impact on the posterior.  
...
```{r, solution_03g_a}
qbeta(0.05, shape1 = a_post_start, shape2 = b_post_start)
qbeta(0.95, shape1 = a_post_start, shape2 = b_post_start)
```
In the first 30 observation, the MLE is not included within the middle 90% uncertainty intervals. That because of the low number of observation and prior belief that govern the posterior. Also, the initial performance of the player was not good. 

```{r, solution_03g_b}
qbeta(0.05, shape1 = a_post_player, shape2 = b_post_player)
qbeta(0.95, shape1 = a_post_player, shape2 = b_post_player)
```
In the 602 at bats, the MLE is within the 90% uncertainity intervals. As the number of observation increased, the data effect on the posterior increased as well. 
...
```{r, solution_03h_a}
Prior_over_0.3 = 1 - pbeta(0.3, shape1 = a_prior, shape2 = b_prior)
Prior_over_0.3 
```


```{r, solution_03h_b}
Post_over_0.3 = 1 - pbeta(0.3, shape1 = a_post_player, shape2 = b_post_player)
Post_over_0.3
```
A over 0.3 hit probability is more likely to happen in 602 observation as the postterior is significantly higher than prior. That shows the posterior is highly impacted by the number of observation. So, seeing high number of observations helps us to more confidently conclude that this player performance is good. 
The posterior shows there is a 65% chance that the player is a good hitter (based on p-posterior is bigger than 0.5), however, there is still 35% chance that the player cannot have a good hit results. 
...
$$
log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto log \left(\mathrm{normal}\left( x_n \mid \mu, \sigma \right) \right) +  log (\mathrm{Exp}\left( \sigma \mid \lambda \right))\\
log (\mathrm{Exp}\left( \sigma \mid \lambda \right)) = log(\lambda\times\exp\left(-\lambda \sigma \right)) = log(\lambda)-\lambda\times\sigma\\

log \left(\mathrm{normal}\left( x_n \mid \mu, \sigma \right) \right) = \sum_{n=1}^{N}((\dfrac{-1}{2\times \sigma ^{2}})(x_n-\mu)^{2} -\dfrac{1}{2}log(\sigma ^{2})-\dfrac{1}{2}log(2\pi) )\\
Drop\ everything\ that\ does\ not\ have\ \sigma : \\
log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto -\lambda\sigma + \sum_{n=1}^{N}((\dfrac{-1}{2\times \sigma ^{2}})(x_n-\mu)^{2}-\dfrac{1}{2}log(\sigma ^{2}))\\
$$
...
#### SOLUTION
$$
\sum_{n=1}^{N}\left( \left(x_n - \mu\right)^2 \right) = N \times v \\
log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto -\lambda\sigma + (\dfrac{-Nv}{2\times \sigma ^{2}}) - Nlog(|\sigma|)\\

$$
Since the sample mean is a representative of information content of all observations, we call it sufficient. the log posterior does not rely on individual observation but it rely on the sample mean. 
...
{
log_post_sigma_unnorm = -lambda*sigma + (-N*v/(2*sigma^(2)))-N*log(abs(sigma))
}
```
...
lambda <- 0.75
tibble::tibble(sigma = seq(0.5, 4.5, by = 0.04 )) %>% 
  mutate(log_post = log_post_sigma_unnorm(sigma, N, v, lambda)) %>% 
   ggplot() +
geom_line(mapping = aes(x = sigma, y = log_post))
```
No, it does not have several modes. It does not have exactly the shape of parabola. 
...
\frac{d}{d\varphi}(g^{-1}(\varphi)) = \frac{d}{d\varphi}(exp (\varphi))= exp (\varphi)\\
log (\frac{d}{d\varphi}(g^{-1}(\varphi))) = log (exp (\varphi)) = \varphi
$$
...
log_post_varphi_unnorm <- function(varphi, N, v, lambda )
{
  ...
  log_post <- -lambda*sigma + (-N*v/(2*sigma^2)) -N*log(abs(sigma)) + varphi
  
  # account for the derivative adjustment
  log_post_varphi_unnorm = log_post + log_post_sigma_unnorm(sigma, N, v, lambda)
}
```
...
lambda <- 0.75
tibble::tibble(varphi = log (seq(0.5, 4.5, by = 0.04 ))) %>% 
  mutate(log_post = log_post_varphi_unnorm(varphi, N, v, lambda))%>% 
 ggplot() +
geom_line(mapping = aes(x = varphi , y = log_post))
```
Yes it does appear more parabolic now with the transformed variable. 