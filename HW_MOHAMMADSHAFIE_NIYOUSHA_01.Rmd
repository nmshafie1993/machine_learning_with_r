...
hist_breaks <- seq(from = 4.2, to = 8.2 , length.out = num_breaks)
```
...
```{r, solution_02c}
x_db1 <- 0:11
### create vector 0 to 11

x_factor <- cut(x_db1, breaks = c(0,5,11)) 
...
```{r, solution_02d}
x_factor_2 <- cut(x_db1, breaks = c(0,5,11), include.lowest = TRUE) 
### set the breaks and add the new argument
...
```{r, solution_02e}
my_iris <- iris
sepal_length_bin <- cut(my_iris$Sepal.Length, breaks = hist_breaks, include.lowest = TRUE)

my_iris$sepal_length_bin <- sepal_length_bin
```
...
#### SOLUTION
Sepal.Length is a continuous variable. The x-axis in histogram shows the variable being measured. In histogram, the columns are not separated by space. Depending on the width of the intervals, the overall shape of histogram might vary. In question 3, the data has been repesented in the form of bar plot. There are some spaces between the bars that might slightly change the intervals. That cause the x-axis of histogram to be different from the bar plot. Also the lower bond and upper bond of historgram is slightly different from the barplot. That will also affect the shape of two plots to be different from each other. 
...
The results from this section are similar to count() results. It seems that count() does the grouping itself while summarise need the data to be grouped before by group_by.  
...
iris_group %>% 
  ...
  geom_point(mapping = aes(y = num_obs),
             size = 4)
```
The stem plot and the manual histogram show the mean of the data while our original histogram shows the distribution of the data. In here since the upper band and lower band are slightly different, the shape of manual historgam will be different from the original one.
...
```
This graphs shows the relation between color and the price of dimond. Based on the boxplot graph, the colors can impact the price. Some colors like 'j' and 'i' have the highest impact which resulted in highest price. However, the mean of the data in all colors seem to be relatively the same. 
...
In 5b, the graph was not showing any information about the clarity or cut of the dimonds. However, in 5f, we filter our dataset to focus on dimonds with specific cut = ideal and clarity = IF. Also, we divided our data based on carat median. Despite the 5b that shows the overall trend for color vs price and color "j" and 'i" had the highest price. Here, we see based on clarity, cut, and carat, the dependency of price of color can change significantly. As we see in 5f, the highest price belongs to D while in overal plot in 5b, the highest price belongs to J and I color. 
...
$$ 
\frac{\partial^{2} f}{\partial x_{1}^{2}}= ab(b-1)x_{1}^{b-2}
$$
...
#### SOLUTION
$$
 Inner product = 1 \times 1 \\
 Outer Product = N \times N
$$
