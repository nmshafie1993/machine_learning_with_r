...
train_data %>% ggplot(mapping = aes(x=x, y = y)) +geom_point()
### 
```
...
### 
train_data %>% ggplot(mapping = aes(x=x, y = y)) +geom_point() + geom_smooth()
```
...
### 
train_data %>% ggplot(mapping = aes(x=x, y = y)) +geom_point() + geom_smooth(span = 0.2)
```
...
$$
\mathbf{H} = g\left(\mathbf{A}\right) = g\left(\mathbf{X}\boldsymbol{\beta}\right)= g(\beta_{0,k}+\sum_{d=1}^{D}x_{n,d}\beta_{1,k})
$$
$$
...
in case that $D=1$
$$
\mathbf{H} = g(\beta_{0,k}+x_{n,1}\beta_{1,k})
$$
Dimensionality of matrix A = [N $\times$ (D+1)]$\times$[(D+1)$\times$ H] = N $\times$ H
### 1c)
...
$$
f = \alpha_{0} + H\boldsymbol{\alpha}
$$
dimensionality of f is = [N $\times$ (H)]$\times$[(H)$\times$ 1] = N $\times$ 1
### 1d)
...
info_two_units$num_params <- info_two_units$num_hidden * ncol(info_two_units$design_matrix) +info_two_units$num_hidden + 1
```
...
optim_fit_2_a$par
```
```{r, solution_01g_d, eval=TRUE}
optim_fit_2_b$par
```
initial guess has no impact on linear and logistic regression but it has impacted on hidden units of neaural networks
## Problem 2
...
$$
\frac{\partial R_{n}}{\partial \alpha_{k}} = \frac{\partial(y_{n} - f_{n})^2}{\partial \alpha_{k}} =  -2(y_{n} - f_{n})\frac{\partial f_{n}}{\partial \alpha_{k}}= -2\delta_{n}\frac{\partial f_{n}}{\partial \alpha_{k}} =  -2(y_{n} - f_{n})h_{n,k}=  -2\delta_{n}h_{n,k}
$$
$$
\frac{\partial R_{n}}{\partial \alpha_{k}}=-2\delta_{n}h_{n,k}
$$
### 2b)
...
$$
\frac{\partial}{\partial \alpha_k} \left(SSE\right) = \sum_{n=1}^{N} \left( \frac{\partial R_n}{\partial \alpha_k} \right)= \sum_{n=1}^{N}-2\delta_{n}h_{n,k} = -2\mathbf{h}_{:,k}^{T}\boldsymbol{\delta}
$$
...
$$
[\sum_{n=1}^{N} \frac{\partial R_n}{\partial \alpha_1}, \sum_{n=1}^{N} \frac{\partial R_n}{\partial \alpha_2}, ... , \sum_{n=1}^{N} \frac{\partial R_n}{\partial \alpha_H}] = [-2\mathbf{h}_{:,1}^{T}\boldsymbol{\delta}, -2\mathbf{h}_{:,2}^{T}\boldsymbol{\delta}, ..., -2\mathbf{h}_{:,H}^{T}\boldsymbol{\delta}]
$$
$$
\frac{\partial}{\partial \alpha_k} \left(SSE\right) = -2\mathbf{H}^{T}\boldsymbol{\delta}
$$
the dimensionality of this vector is H $\times$ 1. yes, $\frac{\partial R_0}{\partial \alpha_0}$ is missing. The output layer of bias is not included.  
### 2d)
...
$$
[\sum_{n=0}^{N} \frac{\partial R_n}{\partial \alpha_0}, \sum_{n=0}^{N} \frac{\partial R_n}{\partial \alpha_2}, ... , \sum_{n=0}^{N} \frac{\partial R_n}{\partial \alpha_H}]
$$
$$
\mathbf{g}_{\alpha} = [-2\mathbf{h}_{:,1}^{T}\boldsymbol{\delta}, -2\mathbf{h}_{:,2}^{T}\boldsymbol{\delta}, ..., -2\mathbf{h}_{:,H}^{T}\boldsymbol{\delta}]
$$
### 2e)
...
$$
\frac{\partial R_n}{\partial \beta_{dk}} =  -2(y_{n} - f_{n})\frac{\partial f_{n}}{\partial \beta_{dk}} 
$$

$$
\frac{\partial f_{n}}{\partial \beta_{dk}} = \alpha_k g_{n,k}^{\prime} x_{n,k}
$$

$$
\frac{\partial R_n}{\partial \beta_{dk}} =  -2(y_{n} - f_{n})\alpha_k g_{n,k}^{\prime} x_{n,k} = -2\alpha_k g_{n,k}^{\prime} \delta_{n}x_{n,k}= -2\xi_{n,k} x_{n,k}
$$
...
$$
$$
\mathbf{G}_{p} (N, H) = g_{N,H}^{\prime} = H \circ (1- H)
$$
### 2g)
...
#### SOLUTION
$$
\mathbf{J} \left(d, k\right) = \frac{\partial}{\partial \beta_{dk}} \left(SSE\right)=\frac{\partial R_n}{\partial \beta_{dk}} 
$$

$$
\frac{\partial R_n}{\partial \beta_{dk}} = -2\alpha_k g_{n,k}^{\prime} \delta_{n}x_{n,k} =-2\xi_{n,k} x_{n,k} =  -2\mathbf{X}^{T}\mathbf{E}
$$
## Problem 3
...
info_11_units$num_params <- info_11_units$num_hidden * ncol(info_11_units$design_matrix) +info_11_units$num_hidden + 1

info_11_units$num_params
```

How many parameters are in the model with 11 units?  34


Do we need to define a new design matrix?  no
...
Based on summary of sample size around 200 used each time to train the model 
Summary of sample sizes: 199, 201, 201, 199, 200 
Therefore, approximately 50 samples used to test the model (250-200 = 50)
layer1 = 5 is the best one, the Rsqured = 0.9906381
...
What's the best model?  The final values used for the model were layer1 = 14, layer2 = 0 and layer3 = 0.

```{r, solution_04c_e, eval=TRUE}
fit_nnet_cv$bestTune
```
...
#### SOLUTION 
I would consider the model with a single layer and 5 hidden units that has error range less than one standard error to be good enough. 